...
module.exports.run = async (client, interaction, author, guild) => {
    ...
    if(await api.getVisitorRole() === role) {
    ...
    if(role === await api.getGuildRole()) {
        let embed = await logger.createErrorEmbed("Configuration Error", `The given role is the same as the GuildMember's role`)
        interaction.reply({embeds: [embed]})
        return;  
    }

    let oldRole = await api.getVisitorRole()
    await api.setVisitorRole(role);
    ...
    for(let i = 0; i < data.length; i++) {
        ...
        if(member) {
            if(await api.isDataGuildMember(user.username) === true) continue;
            affected_users++;
            if(member.roles.cache.get(oldRole)) {
                member.roles.remove(oldRole)
            }
            await link.setGuildRoles(guild.id, user.id, false)
        }
    }
    let embed = await logger.createSuccessEmbed("Roles Updated", "The Visitor's role have been changed!\n` " + affected_users + " ` users have been affected")
    interaction.reply({embeds: [embed]})
    return; 
